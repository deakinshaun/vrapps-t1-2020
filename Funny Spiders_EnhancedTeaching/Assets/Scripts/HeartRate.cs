...
public class HeartRate : MonoBehaviour
{
    ...
    // The number of sensor readings to use for analysis.
    //Larger numbers allow smaller frequency intervals but slow down processing and response times.
    ...
    private float spectroScale = 2.0f;

    // An offset, to center, the spectrum chart
    private float spectroOffset = -2.0f;

    // Number of adjacent bins in the spectrum that are collapsed in a single bar. Speeds up display of the chart.
    ...
    // When searching for peak amplitudes, restrict to frequencies in this range.
    //These ones are tuned to likely heart rate values (in beats per second).
    ...
    void FixedUpdate()
    {
        ...
        {
            // Prepare for the fast fourier transform. Convert all real sensor readings to complex numbers.
