...
public class AccessOpenCV : MonoBehaviour
{
    ...
    private string[] _classes = {"background", "aeroplane", "bycicle", "bird", "boat",
                                    "bottle", "bus", "car", "cat", "chair", "cow", "diningtable", "dog",
                                        "horse", "motorbike", "person", "pottedplan", "sheep", "sofa", "train", "tvmonitor"};
    ...
    [DllImport("VisualRecognition")]
    private static extern void retrieveMatch(int i, ref int category, ref float confidence, ref float sx, ref float sy,
        ref float ex, ref float ey);
        ...
        if (modelReady && (delayTime > 2.0f))
        {
            ...
            for (int i = 0; i < numMatch; i++)
            {
                ...
                if (confidence > 0.2f)
                {
                    ...
                    addVisual(_classes[category], confidence, sx, sy, ex, ey);
                }
            }
        }
    }

    // Copy file from the android package to a readable/writable region of host file system.
    IEnumerator extractFile(string assetPath, string assetFile)
    {
        // Source is the streaming asset path.
